Rocket Ground Station
=====================

PyQt5 製作的火箭地面站 GUI，包含序列埠連線、即時遙測顯示、地圖軌跡、高度曲線與 CSV 記錄控制。統一採用 47-byte 固定長度封包（Header=0xAA55）的解碼，並以 T+ 任務時間呈現飛行狀態。


快速開始
--------
1. 安裝需求：Python 3.10+（已測 PyQtWebEngine 需要 Qt WebEngine 依賴）、可用的 serial 裝置。
2. 安裝套件：
   ```bash
   pip install -r requirements.txt
   ```
3. 啟動地面站：
   ```bash
   python main.py
   ```
4. 在「序列埠」面板選擇正確的 Port/波特率（預設 115200）後按「Connect」。
5. 若系統缺少必要套件，`python3 main.py` 會直接輸出安裝指令並結束（不再噴 traceback）。

獨立測試 GUI
----------
- BMP388 氣壓高度測試：
  ```bash
  python baro_altitude_main.py
  ```
- ICM42688 垂直加速度測試：
  ```bash
  python icm_vertical_accel_main.py
  ```
  此 GUI 讀取韌體輸出的 IMU CSV：
  `imu_time_us,imu_sample,imu_temp_c,accel_x_g,accel_y_g,accel_z_g,gyro_x_dps,gyro_y_dps,gyro_z_dps,imu_failed_reads`
  GUI 會先反轉 Z 軸；開始紀錄時會先收集 100 筆靜止樣本校正 Z 軸基準，校正完成後才開始寫入紀錄，並以 `vertical_accel_mps2 = ((-accel_z_g) - calibration_baseline_accel_z_g) * 9.80665` 顯示與記錄垂直加速度。
  為了避免靜止測試時偶發不合理極端值污染結果，Z 軸會套用 5 筆 median 濾波，且單點跳動超過 3g 時會暫時維持 median；若連續 3 筆都跳動，則接受為新的基準。
- ESP32-CAM OV5640 UART 監測 / 連拍控制：
  ```bash
  python cam_monitor_main.py
  ```
  搭配 `../rocket_cam_ov5640` 韌體使用，預設 UART baud rate 為 `921600`。GUI 送出 `STATUS`、`CAPTURE`、`BURST START <ms>`、`BURST STOP`，相機端回傳 `CAM_STATUS` JSON 與 `CAM_FRAME` JPEG frame，可設定連拍間隔、開始/停止連拍、拍單張與顯示最新預覽。


UART0(COM) 連線檢查（新板 n16r8v）
-------------------------------
1. 主板韌體需使用 `UART0(Serial)` 輸出 47-byte 遙測，波特率 `115200`，且 `USB CDC` 關閉。
2. 以 USB-UART（或板載 USB-to-Serial）連到電腦後，用下列命令確認系統可看到埠：
   ```bash
   python3 -c "import serial.tools.list_ports as p; print('\n'.join(x.device for x in p.comports()))"
   ```
3. 啟動地面站後，選對應 UART0 的埠與 `115200` 連線。
4. 若有收資料但無有效 frame，序列埠面板會顯示：
   `收到資料但非 47-byte 遙測（port/baud 不符）`。


最小自檢命令
----------
```bash
python3 -m unittest discover -s tests -q
python3 main.py
```

使用指南（介面區塊）
------------------
- 序列埠：掃描可用埠，顯示連線狀態/錯誤，Connect/Disconnect 控制。
- 飛行狀態控制：顯示當前狀態；狀態變更為 ASCENT 時會鎖定任務 T0；IDLE 會重置任務時間。
- Sync RTC：將電腦當前時間送到火箭端 RTC（需串口已連線）。
- 記錄控制：開始/停止 CSV 記錄（檔名 `telemetry_<timestamp>.csv`）。狀態會顯示於按鈕旁並寫入事件面板。
- 速度 / 高度：左右分欄顯示 GPS/氣壓的速度與高度。
- 感測器數據：顯示狀態、錯誤碼、開機時間（秒）、落水狀態、伺服電源/角度、經緯度、偏航、衛星數、Roll/Pitch、AccX/Y/Z、GyroX/Y/Z。
- 地圖：Leaflet 地圖（需網路存取 tile），標記當前 GPS 位置並以折線連接歷史點。
- 高度曲線：顯示 30 秒滑動窗口，時間軸隨時間向右移動，最新數據在右端。
- 事件指示器：列出連線、錯誤、記錄狀態等訊息。
- 電壓/時間：電池電壓、任務時間（T+ 格式）。
- 3D 姿態：以 Roll/Pitch/Yaw 即時旋轉火箭模型，座標軸固定在畫面。


遙測封包格式（47 bytes）
----------------------
所有欄位為 little-endian。

| Byte Offset | 長度 | 欄位名稱 | 型態 | 單位 / 說明 | 中文說明 / 用途 |
| ----------- | ---- | -------- | ---- | ----------- | -------------- |
| 0–1 | 2 | Header | `uint16` | 固定 `0xAA55`（bytes: `0x55 0xAA`） | 封包同步頭，用於對齊資料流 |
| 2–3 | 2 | TimeTag | `uint16` | 0.1 s | 開機後時間戳（0.1 秒） |
| 4–7 | 4 | Latitude (GPS) | `int32` | deg × 1e7 | GPS 緯度，地圖定位用 |
| 8–11 | 4 | Longitude (GPS) | `int32` | deg × 1e7 | GPS 經度，地圖定位用 |
| 12–13 | 2 | GPS Altitude (GPS) | `int16` | 0.1 m | GPS 高度 |
| 14–15 | 2 | GPS Speed (GPS) | `int16` | 0.1 m/s | GPS 速度 |
| 16 | 1 | GPS Sat Count (GPS) | `uint8` | 顆 | 可用衛星數 |
| 17–18 | 2 | Roll (IMU) | `int16` | 0.01 deg | 滾轉角，姿態顯示用 |
| 19–20 | 2 | Pitch (IMU) | `int16` | 0.01 deg | 俯仰角，姿態顯示用 |
| 21–22 | 2 | Yaw (IMU) | `uint16` | 0.1 deg | 偏航角，姿態顯示/航向 |
| 23–24 | 2 | GyroX (IMU) | `int16` | 0.1 deg/s | X 軸角速度 |
| 25–26 | 2 | GyroY (IMU) | `int16` | 0.1 deg/s | Y 軸角速度 |
| 27–28 | 2 | GyroZ (IMU) | `int16` | 0.1 deg/s | Z 軸角速度 |
| 29–30 | 2 | AccX (ADXL) | `int16` | 0.01 g | X 軸加速度（高 G） |
| 31–32 | 2 | AccY (ADXL) | `int16` | 0.01 g | Y 軸加速度（高 G） |
| 33–34 | 2 | AccZ (ADXL) | `int16` | 0.01 g | Z 軸加速度（高 G） |
| 35–36 | 2 | Baro Altitude (BMP) | `int16` | 0.1 m | 氣壓高度，用於高度曲線 |
| 37–38 | 2 | Battery | `uint16` | mV | 電池電壓 |
| 39–40 | 2 | Servo Power | `uint16` | mV | 伺服馬達電源電壓 |
| 41–42 | 2 | Servo Angle | `int16` | 0.1 deg | 伺服角度 |
| 43 | 1 | FlightState | `uint8` | 0=TEST,1=IDLE,2=PREFLIGHT,3=ASCENT,4=APOGEE,5=DESCENT,6=LANDED,99=ABORT | 飛行狀態 |
| 44 | 1 | ErrorCode | `uint8` | Bitmask（可多值同時存在），見下表 | 錯誤代碼 |
| 45 | 1 | WaterDetected | `uint8` | 0=否,1=是 | 落水判斷旗標 |
| 46 | 1 | CRC8 | `uint8` | XOR(Byte 0–45) | 校驗碼 |

ErrorCode（bitmask）
-------------------
| Bit | Hex | 條件 | 說明 |
| --- | --- | ---- | ---- |
| 0 | 0x01 | `lora_ready == false` | LoRa 初始化失敗 |
| 1 | 0x02 | `sd_ready == false` | SD 初始化失敗 |
| 2 | 0x04 | `rtc_ready == false` | RTC 初始化失敗 |
| 3 | 0x08 | `status_bmp == false` | BMP388/BMP3XX 初始化失敗 |
| 4 | 0x10 | `status_imu == false` | IMU 初始化失敗 |
| 5 | 0x20 | `status_adxl == false` | ADXL375 初始化失敗 |
| 6 | 0x40 | `flight_state >= PREFLIGHT && !gps_fix_valid` | GPS 無定位/逾時 |
| 7 | 0x80 | `battery_mv == 0` 或（低於門檻） | 電池低電/ADC 無效 |

收到 CRC 正確才會更新 UI。TimeTag 會顯示為開機時間（秒），任務時間以 ASCENT 為 T0 顯示 T+。

資料來源與目前實作
----------------
- IMU（ICM42688）：提供姿態融合輸出（Roll/Pitch/Yaw）與 GyroX/Y/Z。
- ADXL375：提供加速度 AccX/Y/Z（高 G 加速度）。
- BMP388/BMP3XX：提供氣壓（Pa）與氣壓高度（m）。
- SHT31：提供溫度/濕度（僅記錄於機內 SD，不在遙測封包中）。
- BMP388/BMP3XX：氣壓僅記錄於機內 SD，不在遙測封包中。
- RTC（DS3231）：提供 Unix 時間戳（秒），僅存於機內 SD 記錄，可由地面站同步。
- RTC 同步格式：地面站送出 `RTC_SYNC:<unix>`（ASCII，換行結尾），火箭端收到後更新 RTC。
- GPS：經緯度/速度/高度/衛星數（目前韌體端多數仍為 0，待接入 GPS 串流）。
- Battery：目前韌體端預留（mV），若接入 ADC 需填值。

姿態融合與穩定化
----------------
- 姿態融合：使用 Madgwick IMU（gyro + accel）計算 Roll/Pitch/Yaw。
- Yaw 歸零：啟動後第一筆姿態視為零點（yaw offset）。
- Yaw 鎖定：當 |Pitch| ≥ 80° 時暫停更新 yaw，避免接近垂直的 Euler 亂跳。
- Roll/Pitch 平滑：韌體端低通濾波（目前 `smooth_alpha = 0.2`）。


座標系 / 姿態定義
----------------
- 火箭座標系：Z 軸指向鼻錐/天空，X 軸指向火箭右邊，Y 軸指向發射水平方向（前方）。
- 姿態角定義：Roll 繞 Z 軸、Pitch 繞 X 軸、Yaw 繞 Y 軸。

IMU / ADXL 軸向對應（中文說明）
----------------------------
- ICM42688 原始軸：以晶片座標讀取 `accX/accY/accZ`、`gyrX/gyrY/gyrZ`。
- 姿態融合使用軸：為了符合「Roll=繞Z、Pitch=繞X、Yaw=繞Y」的定義，融合時做了軸重排：  
  - `ax_body = accZ`、`ay_body = accX`、`az_body = accY`  
  - `gx_body = gyrZ`、`gy_body = gyrX`、`gz_body = gyrY`
- ADXL375 對應火箭座標系（加速度輸出）：  
  - `AccX = -ADXL_Y`、`AccY = ADXL_X`、`AccZ = ADXL_Z`
- Yaw 校正：啟動後第一筆姿態會作為零點（yaw 歸零）。
- Yaw 鎖定：俯仰接近垂直時（|Pitch| ≥ 80°）暫停更新 yaw，避免數值亂跳。


CSV 記錄
--------
- 檔名：`telemetry_<unix_ts>.csv`
- 欄位：`time, lat, lon, alt, gps_alt, baro_alt, speed, heading, sat, roll, pitch, accx, accy, accz, gyro_x, gyro_y, gyro_z, battery, servo_power, servo_angle, status, water`
- 操作：在「記錄控制」點「開始記錄」啟動，停止後自動關檔。


重播（開發用途）
---------------
`replay.py` 提供 `ReplayWorker`，可將 (timestamp, raw_bytes) 清單以原速或倍速餵入序列處理流程，模擬序列資料來源。可在開發自訂入口，透過 `data_emitted` 連接到 `MainWindow._on_serial_data` 或 `_process_legacy_stream`。


程式結構
-------
- `main.py`：Qt 入口，啟動 `MainWindow`。
- `gui_main.py`：UI 布局與事件處理、資料分發、CSV 記錄控制。
- `protocol.py`：47-byte 固定封包的同步、CRC 驗證與欄位解碼。
- `gui_widgets.py`：速度/高度、遙測面板、記錄控制等組件。
- `serial_comm.py`：序列埠背景讀取執行緒。
- `state_machine.py`：飛行狀態、錯誤碼定義與訊號。
- `plot_panel.py`：高度曲線（滑動窗口）。
- `map_view.py`：Leaflet 地圖與軌跡線。
- `logger.py`：CSV 記錄器。


開發與注意事項
-------------
- 地圖需要網路下載 OSM tile；無網路時地圖空白但其他功能仍可用。
- 目前僅解碼 47-byte 固定封包；若韌體改版需同步更新 `protocol.py` 的 frame 定義與 `gui_main.py` 的 UI 映射。
- 任務時間以 ASCENT 為 T0；切回 IDLE 會重置 T0。
- 預設序列埠/波特率：`COM3`、115200，可在 UI 選擇或修改 `config.py`。
- 若韌體仍以 Serial 傳封包，請在 `rocket_main/src/main.cpp` 設定 `DEBUG_SERIAL = 0`，避免文字除錯輸出干擾封包；切換 LoRa 後可再開啟除錯。
